...
SEARCH_URL = "https://www.linkedin.com/jobs?q=NISM+finance&location=India"


def scrape(save: bool = True) -> list[dict]:
    """
    Scrape LinkedIn for NISM-related jobs.
    Returns a list of raw job dicts.
    """
    logger.info("[LinkedInScraper] Starting scrape...")
    jobs = []
    ...
    if save and jobs:
        save_raw(jobs, "linkedin_" + str(date.today()) + ".json")
    return jobs


if __name__ == "__main__":
    results = scrape()
    print(f"Scraped {len(results)} jobs from LinkedIn")
