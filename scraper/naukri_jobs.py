...
SEARCH_URL = "https://www.naukri.com/jobs?q=NISM+finance&location=India"


def scrape(save: bool = True) -> list[dict]:
    """
    Scrape Naukri for NISM-related jobs.
    Returns a list of raw job dicts.
    """
    logger.info("[NaukriScraper] Starting scrape...")
    jobs = []
    ...
    logger.info(f"[NaukriScraper] Found {len(jobs)} jobs")
    if save and jobs:
        save_raw(jobs, "naukri_" + str(date.today()) + ".json")
    return jobs


if __name__ == "__main__":
    results = scrape()
    print(f"Scraped {len(results)} jobs from Naukri")
